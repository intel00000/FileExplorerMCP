...
**`video_scenes(path, threshold=0.4, max_scenes=200)`** — RO
Detect shot/scene-cut timestamps (ffmpeg's `scene` score; `threshold` 0..1, lower = more cuts) so the model samples where the picture changes rather than blindly. Returns `{path, duration_sec, scene_count, truncated, scenes:[{index, start_sec}]}`, always including `0.0` as the first cut. Feed the `start_sec` values into `video_frames(timestamps=…)` or `video_contact_sheet` for one representative frame per shot. A full-decode pass (the heaviest video op); its time budget **scales with file size** (~30s/GB, never below one GB's worth) rather than the flat `--ffmpeg-timeout`, and frames are downscaled internally to speed it up. No new dependency (PySceneDetect is a possible future upgrade).
...
**`video_frames(path, start=0, step=1.0, count=8, timestamps=null, max_dimension=null, format="png", quality=85, output="inline")`** — RO
Two modes. **Sweep (default):** walk the video from `start`, one frame every `step` seconds, `count` (≤ 64) per call; the result carries a `next_offset` (the `start` for the next page, null at the end) so the model can march through a whole video a window at a time — pair with `ephemeral=true` to keep context flat. **Explicit:** pass `timestamps=[…]` to grab those exact moments (overrides the sweep). Ordered; with `output="inline"` the first list element is a JSON summary (`{…, next_offset}`) and the rest are images, with `output="file"` a single JSON object lists each frame's saved path (plus `next_offset`). Every inline frame is encoded by the vision model in full, so keep `count` modest.
...
**Guarantees.** Every path passes through the resolve-then-contain check (D5), so no tool can touch anything outside the root, including via symlinks. Enumeration tools that walk the tree themselves — `list_dir`, `glob`, `grep` — re-check every yielded path with `is_within` and never descend into a symlinked directory, so a symlink that escapes the root cannot leak even the *names* of outside files (nor cause traversal cycles). On Windows, reserved device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1`-`COM9`, `LPT1`-`LPT9`) are rejected, since they resolve to devices regardless of directory and would otherwise pass a purely lexical containment check. The server is **read-only by default**: the mutating tools are not registered at all unless the operator opts in at launch — `--allow-write` for `write_file`/`make_dir`, `--allow-delete` for `move`/`delete` (or `--allow-all` for both). An unregistered tool is invisible to the model, a stronger guarantee than trusting the host to honor annotations. When enabled, the mutating tools are still annotated for hosts that gate by hint: `move` and `delete` carry `destructiveHint: true`; `delete` additionally requires an explicit `recursive` flag for non-empty directories.

**Resource guards.** A confused or adversarial model could otherwise stall the server with expensive calls, so: every `ffmpeg`/`ffprobe` invocation is bounded by `--ffmpeg-timeout` (default 60 s; on timeout the tool returns an error nudging the model to slow down, since timeouts usually mean too many heavy calls at once); `grep` scans only the first ~2000 characters of each line and abandons the search after a few seconds to defang catastrophic-backtracking (ReDoS) patterns, reporting `stopped: true` with a note; and `output="file"` frames are aged out by `--frames-ttl` (default 1 h) so they don't accumulate. A residual TOCTOU window exists between `resolve()` and use, but every tool resolves immediately before the I/O and the model has no symlink-creating primitive, so it is negligible in this threat model; a complete close would need fd-based no-follow opens (`O_NOFOLLOW`/`openat`), which are not portable to Windows.

**HTTP authentication.** HTTP mode is unauthenticated by default. Setting `--auth-token <token>` (or the `FILEBRIDGE_AUTH_TOKEN` env var, preferred so the secret stays out of the process list) registers a `TokenVerifier` that requires a matching `Authorization: Bearer <token>` on every request — a constant-time-compared shared secret, not OAuth; anything else gets `401`. Auth applies to the HTTP transport only (stdio has no headers). Note the server does **not** enable the SDK's DNS-rebinding / `Host`-header protection (`transport_security` is left unset) — a deliberate choice so local `http://127.0.0.1` testing isn't blocked — so a malicious web page could reach a `127.0.0.1` instance; bind a non-loopback host only behind `--auth-token` or a fronting proxy.
...
5. **Symlink policy.** We follow symlinks and rely on post-resolution containment for *access* (`read_file`/`write_file`/…), but the *enumeration* tools (`list_dir`/`glob`/`grep`) now re-check each result with `is_within` and refuse to descend into symlinked directories — so an escaping link leaks nothing, not even names. A stricter posture (reject symlinks outright, even ones that stay inside the root) remains an option if a deployment wants it.
