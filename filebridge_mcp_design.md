...
**Guarantees.** Every path passes through the resolve-then-contain check (D5), so no tool can touch anything outside the root, including via symlinks. Mutating tools are annotated so hosts can gate them: `move` and `delete` carry `destructiveHint: true`; `delete` additionally requires an explicit `recursive` flag for non-empty directories. These annotations let a host require confirmation or disable destructive tools entirely.

**Deployment cautions.** The server still grants full read/write within the root — point it only at a folder you are willing to fully expose. HTTP mode has **no authentication** in this version; do not expose it on an untrusted network without putting auth in front of it. Even with the sandbox, treat file *contents* as untrusted input to the model (injection risk is a host/model concern, not something the server can neutralize).
